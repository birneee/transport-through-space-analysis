...
# %% info
print(f'without xse: {conn.avg_stream_receive_rate(0)} bit/s')
xse_raw = conn_xse.avg_stream_receive_rate(0)
print(f'with xse: raw: {xse_raw} bit/s')
xse_data = conn_xse.avg_xse_stream_receive_rate(0)
print(f'with xse: payload: {xse_data} bit/s')
overhead_ratio = (xse_raw - xse_data) / xse_data
print(f'with xse: overhead: {overhead_ratio}')
