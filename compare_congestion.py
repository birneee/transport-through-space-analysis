...
def plot(rtt_ms: int, server_side_proxy_handover_ms: int, output_path: str, max_ms = 40000):
    ...
    plot_congestion_window(ax, conn, label='No PEP', color='#253c4b', linestyle='dashed')
    plot_congestion_window(ax, conn_1p, label='Client-side PEP', color='#00885c', linestyle='dashed')
    plot_congestion_window(ax, conn_2p_simple, label='Distributed PEP', color='#ffa600', linestyle='dashed')
    plot_congestion_window(ax, conn_2p, label='Distributed PEP (static CC)', color='tab:orange', linestyle='dashed')
